...
void log_verbose(const char *format, ...) {
  if (quiet_mode || (!verbose_mode && !debug_mode))
    return;
...
void handle_command(int client_fd, const struct AWCCCommand_t *cmd) {
  ...
  case AWCC_CMD_TOGGLE_GMODE:
    ...
    snprintf(response.data, sizeof(response.data),
             "G-Mode toggled - Current mode: %s", mode_name);
  ...
  case AWCC_CMD_GET_FAN_BOOST: {
    enum AWCCFan_t fan =
        (strcmp(cmd->args, "cpu") == 0) ? AWCCFanCPU : AWCCFanGPU;
  ...
  case AWCC_CMD_SET_FAN_BOOST: {
    ...
    if (sscanf(cmd->args, "%15s %d", fan_str, &boost_value) == 2) {
      ...
      if (boost_value >= AWCCFanBoostMin && boost_value <= AWCCFanBoostMax) {
        ...
        response.status = -1;
        snprintf(response.data, sizeof(response.data),
                 "Invalid boost value: %d", boost_value);
        log_debug("Invalid boost value: %d (valid range: %d-%d)", boost_value,
                  AWCCFanBoostMin, AWCCFanBoostMax);
      }
    } else {
      response.status = -1;
  ...
  case AWCC_CMD_FAN_STATUS:
    ...
    snprintf(response.data, sizeof(response.data),
             "%s: %d RPM, %d%% boost\n%s: %d RPM, %d%% boost",
             AWCC.GetFanName(AWCCFanCPU), AWCC.GetFanRpm(AWCCFanCPU),
             AWCC.GetFanBoost(AWCCFanCPU), AWCC.GetFanName(AWCCFanGPU),
  ...
  default:
    response.status = -1;
    snprintf(response.data, sizeof(response.data), "Unknown command: %d",
             cmd->command_type);
  ...
  if (cmd->response_needed) {
    log_debug("Sending response: status=%d, data='%s'", response.status,
              response.data);
