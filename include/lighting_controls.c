...
static const lighting_command_t lighting_commands[] = {
	{LIGHTING_BRIGHTNESS, "brightness", "Set keyboard brightness", "brightness <0-100>", 2, 2},
	{LIGHTING_STATIC, "static", "Set static color", "static <color_hex>", 2, 2},
	{LIGHTING_SPECTRUM, "spectrum", "Color spectrum effect", "spectrum <duration_ms>", 2, 2},
	{LIGHTING_BREATHE, "breathe", "Breathing effect", "breathe <color_hex>", 2, 2},
	{LIGHTING_RAINBOW, "rainbow", "Rainbow wave effect", "rainbow <duration_ms>", 2, 2},
	{LIGHTING_WAVE, "wave", "Wave effect", "wave <color_hex>", 2, 2},
	{LIGHTING_BKF, "bkf", "Back and forth effect", "bkf <color_hex>", 2, 2},
	{LIGHTING_DEFAULTBLUE, "defaultblue", "Default blue color", "defaultblue", 1, 1},
};
...
static int execute_brightness_command(__attribute__((unused)) int argc, char **argv) {
...
static int execute_static_command(__attribute__((unused)) int argc, char **argv) {
...
static int execute_spectrum_command(__attribute__((unused)) int argc, char **argv) {
...
static int execute_breathe_command(__attribute__((unused)) int argc, char **argv) {
	uint32_t color = strtol(argv[1], NULL, 16);
	if (color == 0 && strcmp(argv[1], "0") != 0) {
		fprintf(stderr, "error: invalid color %s\n", argv[1]);
		return 1;
...
static int execute_rainbow_command(__attribute__((unused)) int argc, char **argv) {
	uint16_t duration = strtol(argv[1], NULL, 10);
	if (duration == 0 && strcmp(argv[1], "0") != 0) {
		fprintf(stderr, "error: invalid duration %s\n", argv[1]);
		return 1;
...
static int execute_defaultblue_command(__attribute__((unused)) int argc, __attribute__((unused)) char **argv) {
...
void list_lighting_commands(void) {
	...
	for (int i = 0; i < LIGHTING_COMMAND_COUNT; i++) {
		printf("  %-12s - %s\n", lighting_commands[i].command, lighting_commands[i].description);
	}
}
...
void generate_lighting_help(void) {
	...
	for (int i = 0; i < LIGHTING_COMMAND_COUNT; i++) {
		printf("  %-20s %s\n", lighting_commands[i].usage, lighting_commands[i].description);
	}
	printf("\n");
}