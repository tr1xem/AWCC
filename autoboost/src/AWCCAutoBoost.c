...
static void Start(const struct AWCCConfig_t *,
				  const struct AWCCSystemLogger_t *);
...
void Start(const struct AWCCConfig_t *config,
	...
	while (1) {
		...
		if (Internal.BoostInfos[AWCCFanGPU].Temperature >
		...
		printf("\tCPU %d[%d] GPU %d[%d]\n",
			   Internal.BoostInfos[AWCCFanCPU].Temperature, AWCC.GetCpuBoost(),
			   Internal.BoostInfos[AWCCFanGPU].Temperature, AWCC.GetGpuBoost());
#endif // ENABLE_LOGS
		...
		if (NULL != Internal.SystemLogger) {
			Internal.SystemLogger->LogCpuTemp(
				Internal.BoostInfos[AWCCFanCPU].Temperature);
			Internal.SystemLogger->LogGpuTemp(
				Internal.BoostInfos[AWCCFanGPU].Temperature);
			if (AWCCModeG != Internal.ModeInfo.Mode) {
				Internal.SystemLogger->LogCpuBoost(
					Internal.BoostInfos[AWCCFanCPU].Boost);
				Internal.SystemLogger->LogGpuBoost(
					Internal.BoostInfos[AWCCFanGPU].Boost);
			} else {
		...
		thrd_sleep(
			&(struct timespec){.tv_sec =
								   Internal.Config->TemperatureCheckInterval},
			NULL);
	}
}
...
void ManageFanBoost(enum AWCCFan_t fan) {
	...
	for (int i = 0; i < Internal.Config->FanConfigs[fan]._BoostIntervalCount;
		 i++) {
		if (Internal.Config->FanConfigs[fan]
					.BoostIntervals[i]
					.TemperatureRange.Min <=
				Internal.BoostInfos[fan].Temperature &&
			Internal.Config->FanConfigs[fan]
					.BoostIntervals[i]
	...
	} else if (boostIntervalOfTemperature >
			   Internal.BoostInfos[fan].BoostInterval) {
		Internal.SetFanBoost(fan, boostIntervalOfTemperature, 1);
	} else if (boostIntervalOfTemperature <=
				   Internal.BoostInfos[fan].BoostInterval &&
			   AWCCBoostPhaseUpShift == Internal.BoostInfos[fan].BoostPhase) {
		if (difftime(time(NULL), Internal.BoostInfos[fan].BoostSetTime) >
			Internal.Config->FanConfigs[fan].UpBoostShiftTime) {
			Internal.SetFanBoost(fan, Internal.BoostInfos[fan].BoostInterval,
								 0);
		}
	} else if (boostIntervalOfTemperature <
				   Internal.BoostInfos[fan].BoostInterval &&
			   AWCCBoostPhaseNormal == Internal.BoostInfos[fan].BoostPhase) {
		if (Internal.BoostInfos[fan].Temperature <
			Internal.Config->FanConfigs[fan]
					.BoostIntervals[Internal.BoostInfos[fan].BoostInterval]
					.TemperatureRange.Min -
				Internal.Config->FanConfigs[fan].BoostDownHysteresis) {
			if (difftime(time(NULL), Internal.BoostInfos[fan].BoostSetTime) >
				Internal.Config->FanConfigs[fan].MinTimeBeforeBoostDown) {
				Internal.SetFanBoost(
					fan, Internal.BoostInfos[fan].BoostInterval - 1, 0);
			}
		}
	}
}
...
void ManageMode(void) {
	...
	for (int i = 0; i < Internal.Config->_ModeIntervalCount; i++) {
		if (Internal.Config->ModeIntervals[i].TemperatureRange.Min <=
				Internal.ModeInfo.MaxTemp &&
			Internal.Config->ModeIntervals[i].TemperatureRange.Max >=
				Internal.ModeInfo.MaxTemp) {
	...
	if (AWCCModePhaseInitial == Internal.ModeInfo.ModePhase) {
		...
		if (Internal.ModeInfo.MaxTemp <
			Internal.Config->ModeIntervals[Internal.ModeInfo.ModeInterval]
					.TemperatureRange.Min -
				Internal.Config->ModeDownHysteresis) {
			if (difftime(time(NULL), Internal.ModeInfo.ModeSetTime) >
				Internal.Config->MinTimeBeforeModeDown) {
...
void SetFanBoost(enum AWCCFan_t fan, int boostInterval, _Bool upShift) {
	AWCCBoost_t boost =
		Internal.Config->FanConfigs[fan].BoostIntervals[boostInterval].Boost;
...
#ifdef ENABLE_LOGS
	printf("%s BOOST %3d INTERVAL %d UPSHIFT %d\n", Internal.FanNames[fan],
		   boost, boostInterval, upShift);
#endif // ENABLE_LOGS
