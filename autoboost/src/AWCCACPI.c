...
struct {
	...
	// , .CmdGetGPUBoost      =   "\\_SB.%s.WMAX 0 0x14 {0x0c, 0x33, 0x00,
	// 0x00}"    , .CmdSetCPUBoost      =   "\\_SB.%s.WMAX 0 0x15 {0x02, 0x32,
	// 0x%02x, 0x00}"  , .CmdSetGPUBoost      =   "\\_SB.%s.WMAX 0 0x15 {0x02,
	// 0x33, 0x%02x, 0x00}"  ,
...
void Initialize(
	void) // TODO: fix prefix and cpu/gpu specific codes in acpi calls
{
...
AWCCBoost_t GetFanBoost(enum AWCCFan_t fan) {
	...
	snprintf(cmd, sizeof(cmd), Internal.CmdGetFanBoost, Internal.Prefix,
...
void SetFanBoost(enum AWCCFan_t fan, AWCCBoost_t boost) {
	...
	snprintf(cmd, sizeof(cmd), Internal.CmdSetFanBoost, Internal.Prefix,
			 Internal.FanToBoostHexMap[fan], boost);
	Internal.Execute(cmd);
}
...
AWCCTemperature_t GetFanTemperature(enum AWCCFan_t fan) {
	...
	static _Thread_local char cmd[256];
	snprintf(cmd, sizeof(cmd), Internal.CmdGetTemperature, Internal.Prefix,
...
void SetMode(enum AWCCMode_t mode) {
	static _Thread_local char cmd[256];
	snprintf(cmd, sizeof(cmd), Internal.CmdSetCurrentMode, Internal.Prefix,
