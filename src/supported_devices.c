...
static uint32_t try_acpi_detection(const char *acpi_prefix, bool debug) {
    ...
    if (call_fp) {
            ...
            if (fgets(result, sizeof(result), call_fp)) {
                ...
                if (hex_start) {
                    ...
                    uint32_t parsed_result =
                        (uint32_t)strtoul(hex_start, &endptr, 16);
                    if (debug)
                    ...
                    if (debug)
                        printf(
                            "No hex value found in /proc/acpi/call response\n");
                }
            } else {
                fclose(call_fp);
                if (debug)
        ...
        if (debug)
            printf("Failed to write to /proc/acpi/call (may need root "
                   "permissions)\n");
    }
...
bool is_thermal_mode_supported(const char *mode_name) {
    ...
    if (strcmp(mode_name, "balanced") == 0 ||
        strcmp(mode_name, "balance") == 0 || strcmp(mode_name, "b") == 0) {
...
void build_device_info_string(char *buffer, size_t buffer_size) {
    ...
    if (!g_current_device) {
        ...
        // CPU Information
        offset +=
            snprintf(buffer + offset, buffer_size - offset, "CPU Vendor: ");
        FILE *cpuinfo = fopen("/proc/cpuinfo", "r");
        if (cpuinfo) {
            char line[256];
                ...
                if (strstr(line, "vendor_id")) {
                    if (strstr(line, "AuthenticAMD")) {
                        offset += snprintf(buffer + offset,
                                           buffer_size - offset, "AMD\n");
                    } else if (strstr(line, "GenuineIntel")) {
                        offset += snprintf(buffer + offset,
                                           buffer_size - offset, "Intel\n");
                    } else {
                        offset += snprintf(buffer + offset,
                                           buffer_size - offset, "Unknown\n");
                    }
                    found_vendor = true;
                    break;
                }
    ...
    if (g_current_device->features.has_thermal_modes) {
        ...
        offset += snprintf(buffer + offset, buffer_size - offset,
                           "    - Performance Mode: %s\n",
                           g_current_device->features.thermal_modes.performance
                               ? "Yes"
                               : "No");
        offset += snprintf(
            ...
            g_current_device->features.thermal_modes.battery_saver ? "Yes"
                                                                   : "No");
        offset += snprintf(
            buffer + offset, buffer_size - offset, "    - G-Mode: %s\n",
    ...
    if (g_current_device->features.has_lighting) {
        ...
        offset += snprintf(buffer + offset, buffer_size - offset,
                           "    - Breathing Effect: %s\n",
                           g_current_device->features.lighting.breathing_effect
                               ? "Yes"
                               : "No");
        offset += snprintf(
        ...
        offset += snprintf(buffer + offset, buffer_size - offset,
                           "    - Back & Forth Effect: %s\n",
                           g_current_device->features.lighting.back_forth_effect
                               ? "Yes"
                               : "No");
    }
